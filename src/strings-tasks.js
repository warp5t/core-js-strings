...
function extractNameFromTemplate(value) {
  ...
  for (let i = 0; value.length > i; i += 1) {
    ...
    if (value[i] === '!') break;
    if (permission === true) name += value[i];
  }
  return name;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; str.length > i; i += 1) {
    let index;
    if (arrLower.includes(str[i])) {
      index = (arrLower.indexOf(str[i]) + 13) % 26;
      result += arrLower[index];
    } else if (arrUpper.includes(str[i])) {
      index = (arrUpper.indexOf(str[i]) + 13) % 26;
      result += arrUpper[index];
    } else {
      result += str[i];
    }
  }
